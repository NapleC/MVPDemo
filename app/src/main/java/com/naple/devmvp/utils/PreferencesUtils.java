...
public class PreferencesUtils {
    ...
    public static String PREFERENCE_NAME = "STC";
    ...
    public static void removeAll(Context context) {
        SharedPreferences settings = context.getSharedPreferences(PREFERENCE_NAME, Context.MODE_PRIVATE);
        //查看所有存储在指定标识下的SharedPreferences内的数据
        Map<String, ?> key_Value = (Map<String, ?>) settings.getAll(); //获取所有保存在对应标识下的数据，并以Map形式返回
        //只需遍历即可得到存储的key和value值
        for (Map.Entry<String, ?> entry : key_Value.entrySet()) {
            Loger.debug("获取的key：" + entry.getKey() + "获取的value:" + entry.getValue());
